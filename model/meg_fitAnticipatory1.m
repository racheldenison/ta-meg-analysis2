...
clear x0 x0Perm
nPerms = 100; % 100
for iS = 17:size(data.all.(fitLevel),2) % subjects
    for iP = 1:nPerms
    ...
    filename = sprintf('ModelFit_SeparatePrecueT1T2_%s',dateStr);
    save(filename,'-v7.3')
end
...
%% Polar plot of fitted phase (OLD) 
% for iS = 1 % :size(data.all.(fitLevel),2) % subjects
%     figure
%     subplot 121
%     meg_figureStyle
...
%     for iC = 1:numel(cueLevel)
%         rho = 1;
%         valT = mdlFit.linear2Hz.(cueLevel{iC}).(fitLevel).solution(idx,iS,4);
%         valRad = t2rad(valT);
...
%     title(sprintf('Fitted phase (t/100) %s',und2space(sessionNames{iS})))
% 
%     % --- Save fig ---
%     figTitle = sprintf('%s_TANoise_ITPCFit_DataPrecue_Separate_Linear2Hz_Phase',sessionNames{iS});
%     saveas(gcf,sprintf('%s/%s.png', figDir, figTitle))
% 
%     % --- Save values --- 
%     % mdlFit.linear2Hz.valT_diff(iS) = valT_diff; 
%     mdlFit.linear2Hz.valR_diff(iS) = valR_diff; 
% end

%% Polar plot (group OLD) 
figure
for iS = 1:size(data.all.(fitLevel),2) % subjects
    subplot 131
    % --- Calculate phase difference ---
    valT_diff = abs(mdlFit.linear2Hz.(cueLevel{1}).(fitLevel).solution(idx,iS,4) - mdlFit.linear2Hz.(cueLevel{2}).(fitLevel).solution(idx,iS,4));
    % valR_diff = t2rad(valT_diff);
    valR_diff = abs(circ_dist(t2rad(mdlFit.linear2Hz.(cueLevel{1}).(fitLevel).solution(idx,iS,4)), t2rad(mdlFit.linear2Hz.(cueLevel{2}).(fitLevel).solution(idx,iS,4)))); 
    % --- Plot fitted phases (radians) ---
    for iC = 1
        rho = 1;
        valT = mdlFit.linear2Hz.(cueLevel{iC}).(fitLevel).solution(idx,iS,4);
...
% --- Save fig ---
figTitle = sprintf('Group_TANoise_ITPCFit_DataPrecue_Separate_Linear2Hz_Phase');
saveas(gcf,sprintf('%s/%s.png', figDir, figTitle))
