...
figDir = sprintf('/Users/%s/Dropbox/github/ta-meg-analysis2/model/ITPC_yoked2_fixedFreq/%s',user,dateStr); 
if ~exist(figDir, 'dir')
    mkdir(figDir)
end
        ...
        for iF = 1:numel(fitTypes)
            ...
            switch fitTypes{iF}
                case 'linear2Hz'
                    ...
                    clear fun
                    fun = @(x)meg_objectiveFunction2_fixedFreq(x,dataFit1,dataFit2,t,Fs,paramNames,fitTypes{iF});
                    % --- Do fit ---
                    if o_bads
                ...
                case 'linear'
                    ...
                    % --- Define objective function ---
                    clear fun
                    fun = @(x)meg_objectiveFunction2_fixedFreq(x,dataFit1,dataFit2,t,Fs,paramNamesLinear,fitTypes{iF});
                    % --- Do fit ---
                    if o_bads
...
for iF = 1:numel(fitTypes)
    for iS = 1:size(data.all.(fitLevel),2) % subjects
        ...
        [~,yhat1,yhat2] = meg_objectiveFunction2_fixedFreq(fittedX,dataFit1(t),dataFit2(t),t,Fs,paramNames,fitTypes{iF});
        ...
        switch fitTypes{iF}
            case 'linear2Hz'
                titleText_Fitted = sprintf('Fitted: intercept1 = %0.2f, slope1 = %0.3f, amplitude1 = %0.2f, phase1 = %0.2f\\pi\nintercept2 = %0.2f, slope2 = %0.3f, amplitude2 = %0.2f, phase2 = %0.2f\\pi\nfreq = %0.2f',...
                    ...
                    mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,3),mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,4)/pi,...
                    mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,5),mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,6),...
                    mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,7),mdlFit.(fitTypes{iF}).(fitLevel).solution(idx,iS,8)/pi,...
                    2);
            case 'linear'
...
load('ModelFit_YokedPrecueT1T2_231001.mat')
mdlFitType = 'yoked_fixedFreq'; 
...
for iS = 1:size(mdlFit.linear2Hz.(fitLevel).fval,2) % :size(data.all.(fitLevel),2) % subjects
    ...
    % --- Freq ---
    freq = 2; % mdlFit.linear2Hz.(fitLevel).solution(idx,iS,find(contains(paramNames,'freq'))); 
    % --- Phase in t ---
    ...
    % --- Save fig ---
    figTitle = sprintf('%s_TANoise_ITPCFit_DataPrecue_YokedFixedFreq_Linear2Hz_Phase_PermutationsHistogram_%d',...
        sessionNames{iS},...
        size(mdlFit.linear2Hz.(fitLevel).fval,1));
    saveas(gcf,sprintf('%s/%s.png', figDir, figTitle))
end
